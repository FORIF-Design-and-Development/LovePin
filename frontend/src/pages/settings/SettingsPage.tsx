/**
 * LovePin Settings Page (API Integrated)
...
function MainSettings({ onNavigate }: { onNavigate: (s: SettingsScreen) => void }) {
  // API 스펙으로 변경: currentUser -> myProfile
  const { myProfile, logout } = useApp();
  const navigate = useNavigate();
  // API 스펙으로 변경: notificationsEnabled -> pushEnabled
  const [notifEnabled, setNotifEnabled] = useState(myProfile?.pushEnabled ?? true);
      ...
      <div style={{ padding: '20px 20px 100px' }}>
        ...
        <div className="lp-card" style={{ padding: '20px', marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
            ...
            <div>
              <p style={{ fontSize: 18, fontWeight: 700, color: '#191F28', marginBottom: 2 }}>{myProfile?.nickname}</p>
              <p style={{ fontSize: 13, color: '#8B95A1', marginBottom: 4 }}>{myProfile?.email}</p>
              <span style={{ background: '#F4F6F8', color: '#8B95A1', borderRadius: 6, fontSize: 11, fontWeight: 600, padding: '2px 8px' }}>
                {myProfile?.provider === 'KAKAO' ? '카카오 계정' : '일반 계정'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="lp-card" style={{ marginBottom: 16, overflow: 'hidden' }}>
          ...
          {[
            ...
            { label: '이메일 수정', sub: myProfile?.email, action: () => onNavigate('edit-email') },
            { label: '비밀번호 변경', sub: myProfile?.provider === 'KAKAO' ? '카카오 계정은 변경 불가' : '현재 비밀번호 확인 후 변경', action: () => onNavigate('edit-password') },
          ].map((item, i) => (
            <button
              key={i}
              onClick={item.action}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: 'none', border: 'none', width: '100%', borderTop: i > 0 ? '1px solid #F0F2F4' : 'none', cursor: 'pointer' }}
            >
        ...
        <div className="lp-card" style={{ overflow: 'hidden' }}>
          <button
            ...
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: 'none', border: 'none', width: '100%', cursor: 'pointer' }}
          >
        ...
        <>
          ...
          <div className="modal-center page-enter">
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowLogoutModal(false)} style={{ flex: 1, background: '#F4F6F8', color: '#191F28', border: 'none', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>취소</button>
              <button onClick={handleLogout} style={{ flex: 1, background: '#f76e7e', color: 'white', border: 'none', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>로그아웃</button>
            </div>
          </div>
        </>
      )}
        ...
        <>
          ...
          <div className="modal-center page-enter">
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleDeleteAccount} style={{ flex: 1, background: '#f76e7e', color: 'white', border: 'none', borderRadius: 12, padding: 14, fontSize: 15, fontWeight: 600, cursor: 'pointer' }}>계정 삭제</button>
            </div>
          </div>
        </>
      )}
    ...
    <div style={{ background: '#FAFAFA', minHeight: '100dvh' }}>
      <div style={{ background: 'white', padding: '56px 20px 16px', borderBottom: '1px solid #F0F2F4' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={onBack} style={{ background: 'none', border: 'none', padding: '4px 8px 4px 0', cursor: 'pointer' }}>
      ...
      <div className="page-enter" style={{ padding: '32px 20px' }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto' }}>
            ...
            <button style={{ position: 'absolute', bottom: 0, right: 0, background: '#f76e7e', border: 'none', borderRadius: '50%', width: 26, height: 26, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }}>
        ...
        <button 
          ...
          onClick={() => { onBack(); toast.success('프로필이 수정되었어요'); }}
        >
          저장
        </button>
      </div>
...
function EditEmailScreen({ onBack }: { onBack: () => void }) {
  ...
  const [step, setStep] = useState<'verify' | 'change'>(myProfile?.provider === 'KAKAO' ? 'change' : 'verify');

  if (myProfile?.provider === 'KAKAO') {
    return (
      <div style={{ background: '#FAFAFA', minHeight: '100dvh' }}>
        <div style={{ background: 'white', padding: '56px 20px 16px', borderBottom: '1px solid #F0F2F4' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div className="page-enter" style={{ padding: '40px 20px', textAlign: 'center' }}>
          ...
          <button style={{ width: '100%', background: '#f76e7e', color: 'white', padding: '14px', borderRadius: 12, border: 'none', fontSize: 16, fontWeight: 700, cursor: 'pointer' }} onClick={onBack}>확인</button>
        </div>
      </div>
    );
  }
      ...
      <div className="page-enter" style={{ padding: '32px 20px' }}>
        {step === 'verify' ? (
          <>
            ...
            <button 
              style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', fontSize: 16, fontWeight: 700, cursor: currentPw ? 'pointer' : 'default', background: currentPw ? '#f76e7e' : '#E5E8EB', color: currentPw ? 'white' : '#8B95A1' }} 
          ...
          <div className="page-enter">
            ...
            <button 
              style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', fontSize: 16, fontWeight: 700, cursor: newEmail.includes('@') ? 'pointer' : 'default', background: newEmail.includes('@') ? '#f76e7e' : '#E5E8EB', color: newEmail.includes('@') ? 'white' : '#8B95A1' }} 
              onClick={() => { onBack(); toast.success('이메일이 변경되었어요. 다시 로그인해주세요.'); }} 
  ...
  return (
    <div style={{ background: '#FAFAFA', minHeight: '100dvh' }}>
      ...
      <div className="page-enter" style={{ padding: '32px 20px' }}>
        ...
        <button 
          style={{ width: '100%', padding: '14px', borderRadius: 12, border: 'none', fontSize: 16, fontWeight: 700, cursor: isValid ? 'pointer' : 'default', background: isValid ? '#f76e7e' : '#E5E8EB', color: isValid ? 'white' : '#8B95A1', marginTop: 16 }} 
          onClick={() => { onBack(); toast.success('비밀번호가 변경되었어요. 다시 로그인해주세요.'); }} 
          disabled={!isValid}
        >
          변경 완료
        </button>
